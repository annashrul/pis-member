...
const WithdrawReport = () => {
    ...
    const handleLoadData = async(where)=>{
        ...
        await handleGet(`transaction/withdrawal?perpage=10${where}`,(datum,isLoading)=>{
    ...
    return (
        <div>
            ...
            >
                ...
                <Column title="Nominal" dataIndex="amount" key="amount"  align={"right"}/>
                <Column title="Charge" dataIndex="charge" key="charge"  align={"right"}/>
                ...
                <Column title="Status" dataIndex="status" key="stauts" render={(_,record)=>{
                    return <Tag color={record.status===0?"volcano":(record.status===1?"green":"blue")}>
                        {record.status===0?"Pending":(record.status===1?"Berhasil":"Gagal")}
                    </Tag>
                }} />
