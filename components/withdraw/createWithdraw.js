import {Button,PageHeader,Popconfirm, notification ,Modal, Form, Row, Col,Select, Radio, Input, Card, List, Avatar} from 'antd';
        ...
        >
            ...
            >
                ...
                <Row type="flex"
                    ...
                    <Col md={4} xs={12}>
                        <small style={{fontSize:fontSize}}>Saldo Bonus <br/><span style={{fontSize:"18px",color:theme.primaryColor}}>{Helper.toRp(bonus)}</span></small>
                    </Col>
                    <Col md={4} xs={12}>
                        <small style={{fontSize:fontSize}}>Saldo Bonus Nasional<br/><span style={{fontSize:"18px",color:theme.primaryColor}}>{Helper.toRp(parseFloat(bonusNasional).toFixed(0))}</span></small>
                    </Col>
                </Row>
                <Row>
                    <Col md={24} xs={24} sm={24} className="mb-2"/>
                </Row>
                {step===0&&<Row type="flex"
                    style={{ alignItems: "center" }}
                    justify="center"
                    gutter={4}>
                    <Col md={8} xs={24}>
                        <Card>
                            <Form.Item name="type" label={<small style={{fontSize:fontSize}}>Tipe Withdraw</small>}  onChange={onChange}>
                                <Radio.Group buttonStyle="outline">
                                    <Radio.Button value="0">Bonus</Radio.Button>
                                    <Radio.Button value="1">Bonus Nasional</Radio.Button>
                                </Radio.Group>
                            </Form.Item>
                            <Form.Item hasFeedback name="id_bank" label={<small style={{fontSize:fontSize}}>Bank Tujuan</small>}  rules={[{ required: true,message:"Tidak Boleh Kosong" }]}>
                                <Select
                                    style={{width:"100%"}}
                                    showSearch
                                ...
                                >
                                    {
                                        rekening.map((val,key)=>{
                                            return (
                                                <Option key={key} value={val.id}>{val.acc_name} - {val.acc_no}</Option>
                                            );
                                        })
                                    }
                                </Select>
                            </Form.Item>
                            <Form.Item label={<small style={{fontSize:fontSize}}>Nominal</small>}>
                                <Row gutter={4}>
                                    <Col xs={type==="0"?14:24} sm={type==="0"?14:24} md={type==="0"?16:24}>
                                    <Form.Item hasFeedback onChange={onChange}  name="amount" rules={[
                                        ...
                                        <Input style={{fontSize:state.mobile?"12px":"14px"}} ref={nominalInput} disabled={type==="1"} prefix={"Rp."}/>
                                    </Form.Item>
                                    </Col>
                                    {
                                        type==="0"&&<Col xs={10} sm={10} md={8}>
                                            <Button  onClick={(e)=>{
                                                e.preventDefault();
                                                form.setFieldsValue({amount:bonus});
                                            }} type="dashed" primary style={{width:"100%"}}><small style={{fontSize:fontSize}}>Tarik Semua</small></Button>
                                        </Col>
                                    }
                ...
                {step===1&&<Row type="flex" justify="center" gutter={10}>
                    <Col md={8} xs={24} className={"mb-2"}>
                        ...
                        {tempRow("Tipe Penarikan",type==="0"?"Bonus":"Bonus Nasional",false)}
                        ...
                        <Popconfirm
                            ...
                            cancelText="Batal"
                        >
                            <Button className={"mt-2"} type={"primary"} size={"medium"} style={{width:"100%"}} htmlType="submit"
                            >Simpan</Button>
